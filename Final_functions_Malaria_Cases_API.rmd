...
```{r}
StatesIncidence <- unique(d2$CODE[d2$LEVEL == "UF"])
#Ler a tabela de ADMIn anames- está no drive para fazer o gráfico do jeito que está no script abaixo. Facilita muito
#Merge da tabela 
# Assign names
...
MUN_Plots = foreach(i = 1:length(StatesIncidence)) %do% {
  Plot_Name = paste0("TS_Plot_", StatesIncidence[i])
  assign(Plot_Name, ggplot(data = subset(d2, LEVEL == "MU" & 
                                           CODE == StatesIncidence[i]), #naõ consegui resolver o nome aqui, o script aqui usa tabelas diferentes e usa estados por sigla. 
           ...
           facet_wrap(~NAME) +
           labs(title = paste0("P. vivax and P. falciparum cases in ", StatesIncidence[i]," state by municipality, ", StartYear, "-", EndYear), x = "Year", y = "Number of Cases") + 
...
### State plots
```{r}
    MUN_Plots=foreach(i=1:length(HighIncidenceStates)) %do% {
      Plot_Name=paste0("TS_Plot_",HighIncidenceStates[i])
      assign(Plot_Name, ggplot(data = subset(TS, LEVEL == "UF" & 
