...
##FAZER
-Para todos os municipios, usando escala continua de cores, ver no plot abaixo que usa isso.
- Fazer para todos os municípios mas separando por estado. Tem isso automatizado mais acima.
...
ggplot(aes(x = YEAR, y = RANK, group = STATE, color = STATE), data = subset(dRank, TYPE == "Vivax")) +   
  geom_line(size = 2) + 
  scale_x_continuous(breaks = c(2004:2018)) +
...
##FAZER
- Mudar a ordem dos estados, colocando igual o da figura de linhas. -relevel aqui, ver direito.
- Fazer agregado por mês, mas plotar o eixo y com label de ano mesmo, no gráfico de série temporal temos isso.
```{r}
StartYear <- 2004 # (inferior limit = 2004)
EndYear <- 2018 # (superior limit = 2018)

ggplot(aes(x = YEAR, y = STATE, fill = API), data = subset(dRank, TYPE == "Vivax")) +   
  geom_raster() + 
  scale_x_continuous(breaks = c(2004:2018)) +
...
# Type 1 (default)
streamgraph(data, key="name", value="value", date="year", offset="silhouette")
# Type 2
streamgraph(data, key="name", value="value", date="year", offset="zero")
# Type 3.
streamgraph(data, key="name", value="value", date="year", offset="expand")
 
### Cases contribution per year by state 
```{r}
# Falciparum
streamgraph(dRank[dRank$TYPE == "Falciparum",], key = "NAME", value = "SUMCASES", date = "YEAR", offset = "expand")
# Vivax
streamgraph(dRank[dRank$TYPE == "Vivax",], key = "NAME", value = "SUMCASES", date = "YEAR", offset = "expand")
