...
###### CONFERIR ESSA PARTE
## No banco df tem 499 registros de SEXO == "I" e 2 registros de SEXO == NA. Quando transformo para sivep_strat, 2968 NAs.
## Testar se, transformar I em NA para ver a contagem.
## 984 NAs para BR
## 992 NAs para UF
## 992 NAs para MU
...
### Cumulative plot  
There’s one important change that we’ll need to make. The x-axis will have to be population weighted, i.e. proportional to the population size of the state/municipality.  

Ideally I’d like to have a 4 panel plot:  
Pv in all states in Brazil  
Pf in all states in Brazil  
Pv in all municipalities in Brazil  
Pf in all municipalities in Brazil  
With each panel showing a curve for every year.  

- Create population weight   -- to adapt
```{r}
load("./API_Noti_weeklyAPI_state.Rdata")

# Levels of population sizes is by the limits in each stratum
d2 %>% mutate(MONTH = month(DATE, label=TRUE)) %>% 
