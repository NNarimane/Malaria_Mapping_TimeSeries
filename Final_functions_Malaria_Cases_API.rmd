...
dResi <- getWEEKLY_SIVEP_API(IBGEFilePath, StartYear, EndYear, AgTimeUnit)

```

- Save
```{r}
save(dNoti, dResi, dInfe, file = "API_Noti_Resi_Infe.Rdata")
...
###### Ainda não acertei aqui....eu tinha tentado de outras formas mas sei lá o que houve que perdi algumas coisas que tinha feito...mas de qq jeito não tinha consegui salvar.
ggsave(Cases_TS_BR, filename = , width = , height = , dpi = 300, path = Plot_Folder, device = "png")
...
# Save
dev.copy(png, paste0(Plot_Folder,"P. vivax and P. falciparum API in Brazil ", StartYear, "-", EndYear, ".png"),
         width = 1800, height = 800, units = "px", pointsize = 12,
         res = 100)
dev.off()
...
TS_Plot_State <- ggplot(data = subset(dAB, LEVEL == "UF"), aes(DATE, CASE, color = TYPE)) +
  stat_summary(fun.y = sum, geom = "line") +
  scale_x_date(breaks = "year",
               date_labels = "%Y") +
...
TS_Plot_State <- ggplot(data = subset(dAB, LEVEL == "UF"), aes(DATE, API, color = TYPE)) +
  stat_summary(fun.y = sum, geom = "line") +
  scale_x_date(breaks = "year",
               date_labels = "%Y") +
  facet_wrap(.~NAME, ncol = 3) +
...
- Não consegui salvar ainda. Try to save again. Doing  -- AJUDA
```{r}
