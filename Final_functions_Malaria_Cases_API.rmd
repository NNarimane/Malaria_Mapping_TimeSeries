...
} else{Plot_Folder = "/home/rlana/Plos_new/TimeSeries/STATE"} # Raquel
...
#  scale_fill_gradient(low = "blue", high =  "darkblue") +
  ...
  labs(title = paste0("P. vivax cases intensity stratified by age and gender in ", "RO", " ",  
  #            StartYear, "-", EndYear), x = "Age categories", y = "Cases intensity by year") +  
              StartYear, "-", EndYear), x = "Cases intensity by year", y = "Age categories") +  
  guides(color = guide_legend(title = "Cases Intensity")) +
  theme(panel.grid.minor.x = element_blank(),
        axis.text.x = element_text(angle = 90, hjust = 1),
...
Age_Gender_Matrix_State_Year <- foreach(i = 1:length(States_sivep)) %do% {
  Plot_Name = paste0("AG_Matrix_", States_sivep[i])
  assign(Plot_Name, ggplot(data = subset(dAge, LEVEL == "UF" & STATE == States_sivep[i] & TYPE == "Falciparum"), 
                           aes(x = YEAR, y = AGE_CAT, fill = SUMCASES))) +
                          geom_raster() + 
                          facet_grid(GENDER ~ .) +
                          scale_x_continuous(breaks = c(2004:2018)) +
#                          scale_fill_distiller(palette = "Blues") + # Vivax
                          scale_fill_distiller(palette = "Greens") + # Falciparum
                          labs(title = paste0("P. falciparum cases intensity stratified by age and gender in ", States_sivep[i], " ",  
                          #            StartYear, "-", EndYear), x = "Age categories", y = "Cases intensity by year") +  
                                      StartYear, "-", EndYear), x = "Cases intensity by year", y = "Age categories") +  
                          guides(color = guide_legend(title = "Cases Intensity")) +
                          theme(panel.grid.minor.x = element_blank(),
...
ggsave(Age_Gender_Matrix_State_Year[[i]], filename = paste0("P. falciparum cases stratified by age and gender in ", States_sivep[[i]], " ", StartYear, "-", EndYear, ".png"), width = 2500/300, height = 2000/300, dpi = 300, path = Plot_Folder, units = "in", device = "png") 
