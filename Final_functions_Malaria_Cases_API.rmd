...
for (uf in States_sivep){
  plt <- ggplot(data = dAge[dAge$LEVEL == "UF" & dAge$STATE == uf & dAge$TYPE == tipo,], 
    ...
    labs(title = paste0("P."," ", tipomin," ", "cases intensity stratified by age and gender in ", uf, " ",  
                        StartYear, "-", EndYear), x = "Cases intensity by year", y = "Age categories") +  
    guides(fill = guide_colourbar(title = "Cases Intensity", reverse = T)) + ### Decidir se quer deixar o reverse T ou F- falci está T
...
dAgen <- dAge %>%
  mutate() %>% 
  summarise() #ou será qua dá pra usar aqui direto?
...
library(dplyr)
#my_data %>% group_by(category) %>% summarise_each(funs(max)) # max por sum aqui
...
#                            "Other"                 = "No answer",         ##Talvez isso resolva, mas acho que tem que somar depois as linhas
#                            "Other"                 = "Don't know",
#                            "Other"                 = "Other party"
#   ))
