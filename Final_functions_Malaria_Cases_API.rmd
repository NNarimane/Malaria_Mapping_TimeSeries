...
```{r}
Plot_Folder <- "/home/rlana/Plots/Age_Gender/Matrix/Vixax" #Falciparum
States_sivep <- c("AC","AM","AP","MA","MT","PA","RO","RR","TO")
tipo <- "Vivax" #Falciparum
tipomin <- "vivax" #falciparum

for (uf in States_sivep){
  Plot_Name = paste0("AG_Matrix_", uf)
...
#    scale_fill_distiller(palette = "Greens") + # Falciparum
    labs(title = paste0("P."," ", tipomin," ", "cases intensity stratified by age and gender in ", uf, " ",  
                        #            StartYear, "-", EndYear), x = "Age categories", y = "Cases intensity by year") +  
                        StartYear, "-", EndYear), x = "Cases intensity by year", y = "Age categories") +  
#    guides(color = guide_legend(title = "Cases Intensity")) +
    theme(panel.grid.minor.x = element_blank(),
