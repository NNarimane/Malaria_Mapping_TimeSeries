...
- Sobre cortar o ano de 2003- decidir   
Usando essa função, restam alguns registros como ano de 2003 pois ao usar a função year do lubridate para extrair o ano pela data de notificação, tem semana epi de 2003.
...
#    drop_na(SEXO, AGE_CAT) %>% 
    ...
    mutate(Falciparum = FALCI + VF,
           Vivax = VIVAX + VF,
           DATE = floor_date(as.Date(DT_NOTIF, format = "%Y-%m-%d"), unit = AgTimeUnit)  # ainda continua com 496 I e 2 NA até aqui
           ) %>%
    ungroup %>%
    select(-DT_NOTIF) %>%
    mutate(GENDER = as.factor(case_when(GENDER == "F" ~ "Female",
                                        GENDER == "M" ~ "Male",
                                        TRUE ~ "NA"))) %>%   # 498 NA após transformação de I em NA.
    select(DATE, LEVEL, CODE, AGE_CAT, GENDER, Falciparum, Vivax) %>%
    gather(key = TYPE, value = CASES, -c(DATE, LEVEL, CODE, AGE_CAT, GENDER))
  
  # Municipality
  # day_malaria_type_age_gender <- day_malaria_type_age_gender %>%
