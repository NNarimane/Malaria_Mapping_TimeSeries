...
Age_Gender_Plot_State_Year <- foreach(i = 1:length(States_sivep)) %do% {
  ...
  assign(Plot_Name, ggplot(data = subset(Sivep_strat, LEVEL == "UF" & STATE == States_sivep[i] & TYPE == "Falciparum"), 
                           aes(x = AGE_CAT, y = CASES, fill = GENDER))) +
           stat_summary(fun.y = sum, geom = "bar", position = "dodge") +
           scale_fill_manual(values = c("#31a354","#3182bd")) +
           facet_wrap(.~YEAR, ncol = 3) +
           labs(title = paste0("P. falciparum cases stratified by age and gender in ", States_sivep[i], " ",  
...
ggsave(Age_Gender_Plot_State_Year[[i]], filename = paste0("P. falciparum cases stratified by age and gender in ", States_sivep[[i]], StartYear, "-", EndYear, ".png"), width = 2500/300, height = 2000/300, dpi = 300, path = Plot_Folder, units = "in", device = "png")    
  }
