...
Date_Type = "Weekly"
Plot_Folder = "/home/rlana/Plots/TimeSeries/"
...
UF_Plots <- foreach(i = 1:length(States_code)) %do% {
  Plot_Name = paste0("TS_Plot_", States_code[i])
  assign(Plot_Name, ggplot(data = subset(d2, LEVEL == "UF" & CODE == States_code[i]), 
...
ggsave(UF_Plots[[9]], filename = paste0("P. vivax and P. falciparum cases in Brazil per State", StartYear, "-", EndYear, "_", States_code[9], ".png"), width = 2500/300, height = 2000/300, dpi = 300, path = Plot_Folder, units = "in", device = "png")  



UF_PlotsAPI <- foreach(i = 1:length(States_code)) %do% {
  Plot_Name = paste0("TS_Plot_API", States_code[i])
  assign(Plot_Name, ggplot(data = subset(d2, LEVEL == "UF" & CODE == States_code[i]), 
...
#ggsave(UF_PlotsAPI[[i]], filename = paste0("P. vivax and P. falciparum API in Brazil per State", StartYear, "-", EndYear, "_", States_code[[i]], ".png"), width = 2500/300, height = 2000/300, dpi = 300, path = Plot_Folder, units = "in", device = "png")  
         
}


# Save
ggsave(UF_PlotsAPI[[9]], filename = paste0("P. vivax and P. falciparum API in Brazil per State", StartYear, "-", EndYear, "_", States_code[[9]], ".png"), width = 2500/300, height = 2000/300, dpi = 300, path = Plot_Folder, units = "in", device = "png")  

```

### Municipalities plots for each state
####Rever junto com o de cima
...
- join with names
```{r}
load("/home/rlana/Dados/Age_gender_Noti_weekly.Rdata")
...
Age_Gender_Plot_State <- ggplot(data = subset(Sivep_strat, LEVEL == "UF" & TYPE == "Vivax"), aes(x = AGE_CAT, y = CASES, fill = GENDER)) + ###testar
  stat_summary(fun.y = sum, geom = "bar", position = "dodge") +  #está somando de todos os anos? Testar.
  facet_wrap(.~NAME, ncol = 3) +
  scale_color_manual(values = c("#31a354","#3182bd")) + ## Eu usei as mesmas cores do plot de série temporal mas não ficou...
  labs(title = paste0("P. vivax cases stratified by age and gender, ", 
...
# Save
ggsave(Age_Gender_Plot_State, filename = paste0("P. vivax cases in Brazil per State", StartYear, "-", EndYear, ".png"), width = 2500/300, height = 2000/300, dpi = 300, path = Plot_Folder, units = "in", device = "png")

```


Plot  age and gender per state per year - automatizar
```{r}  
#Removendo NA de gender
#Sivep_strat <- Sivep_strat %>% 
#  drop_na(GENDER)

Age_Gender_Plot_State_Year <- ggplot(data = subset(Sivep_strat, CODE == 11 & TYPE == "Falciparum"), aes(x = AGE_CAT, y = CASES, fill = GENDER)) + ###testar
  stat_summary(fun.y = sum, geom = "bar", position = "dodge") +  #está somando de todos os anos? Se usar a linha de baixo mudam os valores.
...
Age_Gender_Plot_State_Year

# Save
ggsave(Age_Gender_Plot_State, filename = paste0("P. falciparum cases in Brazil per State", StartYear, "-", EndYear, ".png"), width = 2500/300, height = 2000/300, dpi = 300, path = Plot_Folder, units = "in", device = "png")


States_code = c(11, 12, 13, 14, 15, 16, 17, 21, 51)  

Age_Gender_Plot_State_Year <- foreach(i = 1:length(States_code)) %do% {
  Plot_Name = paste0("AG_Plot_", States_code[i])
  assign(Plot_Name, ggplot(data = subset(Sivep_strat, LEVEL == "UF" & CODE == States_code[i] & TYPE == "Falciparum"), 
                           aes(x = AGE_CAT, y = CASES, fill = GENDER))) +
           stat_summary(fun.y = sum, geom = "bar", position = "dodge") +
           scale_color_manual(values = c("#31a354","#3182bd")) +
           facet_wrap(~YEAR) +
           labs(title = paste0("P. falciparum cases stratified by age and gender from ", Sivep_strat$NAME[[i]],
                               StartYear, "-", EndYear), x = "Year", y = "Number of Cases") + 
           guides(color = guide_legend(title = "Plasmodium")) +
            theme(panel.grid.minor.x = element_blank(),
                  axis.text.x = element_text(angle = 90, hjust = 1),
