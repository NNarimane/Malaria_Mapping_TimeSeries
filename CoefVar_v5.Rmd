...
```
Now investigating only at the state level.  Not sure what happened to
the states RO, RR, TO.
...
An intuition here is that the potential number behaves close to an expected new flow of cases or a next expected IPA from $C_{i,y}$ cases and a population of $N_{i,y}$.
...
Following this approach, when divided by $N_{i,y}$, 
we have $P_{i,y} = C_{i,y} (1- C_{i,y}/N_{i,y})$. 
