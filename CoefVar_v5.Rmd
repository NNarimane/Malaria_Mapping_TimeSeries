...
if(envNN){
  load("./API_Noti_weeklyAPI_state.Rdata") # Daniel
} else{load("/home/rlana/Dados/API_Noti_weeklyAPI_state.Rdata")} # Raquel
...
dNoti %>% group_by(LEVEL, YEAR, POP_SIZE, NAME, TYPE) %>% 
  summarise(SUMCASES = sum(CASES, na.rm=TRUE),
    MEAN = mean(API, na.rm=TRUE), VAR = var(API, na.rm=TRUE)) %>% ####Aqui eu acho que os NAs influenciam- teria que atribuir zero pelo menos uma vez para o município, pois está agregando por ano, entao bastaria ter o município, naquela ano, uma vez, com zero.
...
dNoti %>% group_by(LEVEL, MONTH, YEAR, POP_SIZE, NAME, TYPE) %>% 
  ...
  drop_na(TYPE) %>% group_by(LEVEL, MONTH, TYPE) %>% summarise(MEANAPI = mean(APIN, na.rm=TRUE),   ####Aqui tb eu acho que os NAs impactam. Agregou por mês, teria que aparecer 12 vezes. Mas isso fosse resolvido na origem, que é a gregação por semana, já resolve tudo. PEnso que talvez seja o melhor.
                                                        VARAPI = var(APIN, na.rm=TRUE)) %>%
...
dNoti %>% group_by(LEVEL, STATE, YEAR, POP_SIZE, NAME, TYPE) %>% 
  summarise(SUMCASES = sum(CASES, na.rm=TRUE),
            MEAN = mean(API, na.rm=TRUE), VAR = var(API, na.rm=TRUE)) %>% 
  mutate(APIN = 1000*12*SUMCASES/POP_SIZE) %>%   ##### Vc agrega por ano, não precisa multiplicar por 12.
...
ggplot(aes(x=CVAR, y=MEANAPI, shape=NAME, color=NAME), data=dNoti.lpp) + 
  geom_point() + facet_grid(TYPE ~ .)

```

Now investigating only at the state level. Not sure what happened to the states RO, RR, TO.  
...
ggplot(aes(x=CVAR, y=MEANAPI, shape=STATE, color=STATE), data=dNoti.lpp) + 
...
ggplot( aes(x=CWPOP, y = CPOTN, color=YEAR, group=YEAR), data=subset(dNoti.sx.3, TYPE=="Falciparum")) +
  ...
  xlab("Adjusted number of municipalities")+
  geom_step() + facet_wrap(STATE ~ ., scales = "free") + scale_x_log10() + ggtitle("A. Falciparum")

ggplot( aes(x=CWPOP, y = CPOTN, color=YEAR, group=YEAR), data=subset(dNoti.sx.3, TYPE=="Vivax")) +
  ylab("Cummulative Potential (normalized)") +
  xlab("Adjusted number of municipalities")+
  geom_step() + facet_wrap(STATE ~ ., scales = "free") + scale_x_log10() + ggtitle("B. Vivax")
