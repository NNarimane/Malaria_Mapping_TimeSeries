...
#load("~/Documentos/PosDoc_PROCC/OwncloudPD/malaria/AnalisesRaquel/Pasteur/RData_full.RData")
load("/home/rlana/Dados/RData_full_new.RData")

```

### Removing variables  
**Obs.**: RES_EXAM = 0 happens only in 2003 in 17 entries. I removed because this category does not exist.  
- 5972938 notifications from 2003 to 2018. 5972938 - 17 wrong entries in 2017 = 5972921
...
df %>% count()
df %>% filter(RES_EXAM != "0") %>% count()
# % of malaria types
df %>% filter(RES_EXAM != "0") %>% 
  mutate(RES_EXAM = as.factor(RES_EXAM))

tmal <- df %>%
  filter(RES_EXAM != "0") %>% 
  mutate(RES_EXAM = if_else(RES_EXAM == "2" | RES_EXAM == "3" | RES_EXAM == "7", "F", 
          if_else(RES_EXAM == "4", "V",
                  if_else(RES_EXAM == "5" | RES_EXAM == "6", "F+V",
...
df <- df %>% 
      mutate(RES_EXAM = if_else(RES_EXAM == "2" | RES_EXAM == "3" | RES_EXAM == "7" | RES_EXAM == "9", "F", 
                                if_else(RES_EXAM == "4" | RES_EXAM == "6", "V",
                                        if_else(RES_EXAM == "5", "F+V", RES_EXAM)))) %>% #confirm if "9" is a mixed too
