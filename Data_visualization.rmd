...
### Date of 1st symptoms relationed to date of exam and start of treatment
...
Date_diff <- df %>%
              mutate(DIF.SYM.EXAM = DT_EXAME - DT_SINTO,
                     DIF.EXAM.TREAT = DT_TRATA - DT_EXAME,
                     DIF.SYM.TREAT = DT_TRATA - DT_SINTO)
...
Plot
```{r}
diff1 <- ggplot(aes(y = M.SYM.EXAM, x = ANO), data = Date_diff) + #, fill = GENDER
