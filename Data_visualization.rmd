...
The examination was done before the first symptoms is not a problem, it may be related to active detection. Check this out. See also symptomatic when this happens.
```{r}
summary(df$DT_SINTO)
summary(df$DT_EXAME)
summary(df$DT_TRATA)
...
Date_diff %>% filter(DIF.SYM.EXAM > 3) %>% head(5)
...
Plot
```{r}
s1 <- Date_diff %>% filter(DIF.SYM.EXAM >= 0) 




diff1 <- ggplot(aes(y = DIF.SYM.EXAM, x = ANO), data = Date_diff) + #, fill = GENDER
            geom_histogram(stat = "identity") + #stat = "identity", position = "dodge"
          #  facet_wrap(~ANO) +
            labs(x = "Age categories", y = "Malaria vivax API", title = "Amazonas") + 
            theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) +
            guides(fill = guide_legend(title = "Gender")) 
